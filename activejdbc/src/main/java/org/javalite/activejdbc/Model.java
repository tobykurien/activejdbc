...
public abstract class Model extends CallbackSupport implements Externalizable {
  ...
  public static <T extends Model> MetaModel getMetaModel(Class<T> claz) {
    Registry.instance().init(MetaModel.getDbName(claz));
    return Registry.instance().getMetaModel(getTableName(claz));
  }
  ...
  protected void hydrate(Map attributesMap) {
    ...
    for (String attrName : attributeNames) {
      ...
      if (value instanceof Clob && getMetaModelLocal().cached()) {
        ...
        this.attributes
            .put(attrName, getMetaModelLocal().getDialect().overrideDriverTypeConversion(getMetaModelLocal(), attrName, value));
      }
    }
  }
  ...
  public static <T extends Model> List<String> attributes(Class<T> cls) {
    return getMetaModel(cls).getAttributeNames();
  }
  ...
  public static <T extends Model> List<Association> associations(Class<T> cls) {
    return getMetaModel(cls).getAssociations();
  }
  ...
  private void deleteMany2ManyDeep(List<Many2ManyAssociation> many2ManyAssociations) {
    ...
    for (Association association : many2ManyAssociations) {
      ...
      if (c == null) {// this model is probably not defined as a class, but the table exists!
        logger.error("ActiveJDBC WARNING: failed to find a model class for: " + targetTableName
            + ", maybe model is not defined for this table?" +
  ...
  public static int delete(String query, Object... params) {
    ...
    int count = params == null || params.length == 0 ? new DB(metaModel.getDbName()).exec("DELETE FROM " + metaModel.getTableName()
        + " WHERE " + query) :
  ...
  public static <T extends Model> int delete(Class<T> cls, String query, Object... params) {
  ...
  public static <T extends Model> boolean exists(Class<T> cls, Object id) {
    MetaModel metaModel = getMetaModel(cls);
    return null != new DB(metaModel.getDbName()).firstCell("SELECT " + metaModel.getIdName() + " FROM " + metaModel.getTableName()
        + " WHERE " + metaModel.getIdName() + " = ?", id);
  }
  ...
  public static <T extends Model> int update(Class<T> cls, String updates, String conditions, Object... params) {
    //TODO: validate that the number of question marks is the same as number of parameters
    return ModelDelegate.update(Model.getMetaModel(cls), updates, conditions, params);
  }
  ...
  public String toXml(int spaces, boolean declaration, String... attrs) {
    ...
    for (String name : modelMap.keySet()) {
      ...
      if ((attrList.contains(name) || attrs.length == 0) && !(value instanceof List)) {
        sw.write(indent + indent + "<" + name + ">" + XmlEntities.XML.escape(value.toString()) + "</" + name + ">"
            + (spaces > 0 ? "\n" : ""));
      }
        ...
        for (Map child : children) {
          ...
          for (Object childKey : child.keySet()) {
            sw.write(indent + indent + indent + indent + "<" + childKey + ">" + XmlEntities.XML.escape(child.get(childKey).toString())
                + "</" + childKey + ">" + (spaces > 0 ? "\n" : ""));
          }
          sw.write(indent + indent + indent + "</" + Inflector.singularize(name) + ">" + (spaces > 0 ? "\n" : ""));
        }
  ...
  public <T extends Model> T parent(Class<T> parentClass) {
    ...
    BelongsToAssociation ass = (BelongsToAssociation) getMetaModelLocal()
        .getAssociationForTarget(parentTable, BelongsToAssociation.class);
    ...
    if (ass != null) {
      ...
      if (!assP.getTypeLabel().equals(getString("parent_type"))) {
        throw new IllegalArgumentException("Wrong parent: '" + parentClass + "'. Actual parent type label of this record is: '"
            + getString("parent_type") + "'");
      }
    }
    else {
  ...
  public void setParent(Model parent) {
    ...
    for (Association association : associations) {
      ...
      if (association instanceof BelongsToPolymorphicAssociation
          && association.getTarget().equals(parent.getMetaModelLocal().getTableName())) {
  ...
  protected MetaModel getMetaModelLocal() {
    if (metaModelLocal == null)
      metaModelLocal = getMetaModel(getClass());
   ...
   */
  public void refresh() {
    Model fresh = findById(getClass(), getId());
  ...
  private <T extends Model> LazyList<T> get(String targetTable, String criteria, Object... params) {
    ...
    OneToManyAssociation oneToManyAssociation = (OneToManyAssociation) getMetaModelLocal().getAssociationForTarget(targetTable,
        OneToManyAssociation.class);
    Many2ManyAssociation manyToManyAssociation = (Many2ManyAssociation) getMetaModelLocal().getAssociationForTarget(targetTable,
        Many2ManyAssociation.class);

    OneToManyPolymorphicAssociation oneToManyPolymorphicAssociation = (OneToManyPolymorphicAssociation) getMetaModelLocal()
        .getAssociationForTarget(targetTable, OneToManyPolymorphicAssociation.class);
    ...
    if (oneToManyAssociation != null) {
      ...
      subQuery = "parent_id = " + getId() + " AND " + " parent_type = '" + oneToManyPolymorphicAssociation.getTypeLabel() + "'"
          + additionalCriteria;
    }
    else {
  ...
  public static <T extends Model> ValidationBuilder addValidator(Class<T> cls, Validator validator) {
    return ValidationHelper.addValidator(cls.getName(), validator);
  }
  ...
  public static void removeValidator(Validator validator) {
    ValidatorRegistry.instance().removeValidator(Model.<Model> getDaClass(), validator);
  }

  public static <T extends Model> void removeValidator(Class<T> cls, Validator validator) {
    ValidatorRegistry.instance().removeValidator(cls, validator);
  }

  public static List<Validator> getValidators(Class<Model> daClass) {
    return ValidatorRegistry.instance().getValidators(daClass.getName());
  }
  ...
  public static boolean belongsTo(Class<? extends Model> targetClass) {
    ...
    MetaModel metaModel = getMetaModel();
    return (null != metaModel.getAssociationForTarget(targetTable, BelongsToAssociation.class) || null != metaModel
        .getAssociationForTarget(targetTable, Many2ManyAssociation.class));
  }

  public static boolean belongsTo(Class<? extends Model> parentClass, Class<? extends Model> targetClass) {
    String targetTable = Registry.instance().getTableName(targetClass);
    MetaModel metaModel = getMetaModel(parentClass);
    return (null != metaModel.getAssociationForTarget(targetTable, BelongsToAssociation.class) || null != metaModel
        .getAssociationForTarget(targetTable, Many2ManyAssociation.class));
  }
  ...
  public static void addCallbacks(CallbackListener... listeners) {
    for (CallbackListener listener : listeners) {
      ListenerRegistry.LISTENER_REGISTRY.addListener(getDaClass(), listener);
    }
  }
  ...
  public void validate() {
    ...
    errors = new Errors();
    List<Validator> theValidators = ValidatorRegistry.instance().getValidators(getClass().getName());
  ...
  public static <T extends Model> T create(Class<T> cls, Object... namesAndValues) {
  ...
  public static <T extends Model> T createIt(Class<T> cls, Object... namesAndValues) {
    T m = (T) create(cls, namesAndValues);
    m.saveIt();
    return m;
  }
  ...
  public static <T extends Model> T findById(Class<T> cls, Object id) {
    if (id == null)
      return null;

    MetaModel mm = getMetaModel(cls);
    LazyList<T> l = new LazyList<T>(mm.getIdName() + " = ?", new Object[] { id }, mm).limit(1);
    return l.size() > 0 ? l.get(0) : null;
  ...
  public static <T extends Model> LazyList<T> where(Class<T> cls, String subquery, Object... params) {
    return find(cls, subquery, params);
  }
  ...
  public static <T extends Model> LazyList<T> find(Class<T> cls, String subquery, Object... params) {
    ...
    return new LazyList(subquery, params, getMetaModel(cls));
  }
  ...
  public static <T extends Model> T findFirst(Class<T> cls, String subQuery, Object... params) {
    LazyList<T> results = new LazyList<T>(subQuery, params, getMetaModel(cls)).limit(1);
    return results.size() > 0 ? results.get(0) : null;
  }
  ...
  public static <T extends Model> void findWith(Class<T> cls, final ModelListener listener, String query, Object... params) {
    long start = System.currentTimeMillis();
    final MetaModel metaModel = getMetaModel(cls);
    String sql = metaModel.getDialect().selectStar(metaModel.getTableName(), query);
   ...
   *     .findBySQL(&quot;select rule.*, goal_identifier from rule, goal where goal.goal_id = rule.goal_id order by goal_identifier asc, rule_type desc&quot;);
   * </pre>
  ...
  public static <T extends Model> LazyList<T> findBySQL(Class<T> cls, String fullQuery, Object... params) {
    return new LazyList<T>(false, getMetaModel(cls), fullQuery, params);
  }
  ...
  public static <T extends Model> LazyList<T> findAll(Class<T> cls) {
    return new LazyList(null, new Object[] {}, getMetaModel(cls));
  }

  /**
   * Adds a new child dependency. This method works for all three association types:
   * <ul>
   * <li>One to many - argument model should be a child in the relationship. This method will immediately set it's ID as a
   * foreign key on the child and will then save the child.</li>
   * <li>Many to many - argument model should be the other model in the relationship. This method will check if the added child
   * already has an ID. If the child does have an ID, then the method will create a link in the join table. If the child does
   * not have an ID, then this method saves the child first, then creates a record in the join table linking this model instance
   * and the child instance.</li>
  ...
  public void add(Model child) {
    ...
    if (getId() != null) {
      ...
      if (metaModel.hasAssociation(childTable, OneToManyAssociation.class)) {
        ...
        if (joinMM == null) {
          new DB(metaModel.getDbName()).exec("INSERT INTO " + join + " ( " + sourceFkName + ", " + targetFkName + " ) VALUES ( "
              + getId() + ", " + child.getId() + ")");
        }
        else {
      ...
      throw new IllegalArgumentException(
          "You can only add associated model to an instance that exists in DB. Save this instance first, then you will be able to add dependencies to it.");
    }
  }

  /**
   * Removes associated child from this instance. The child model should be either in belongs to association (including
   * polymorphic) to this model
   * or many to many association.
   * 
   * <h3>One to many and polymorphic associations</h3> This method will simply call <code>child.delete()</code> method. This
   * will render the child object frozen.
   * 
   * <h3>Many to many associations</h3> This method will remove an associated record from the join table, and will do nothing to
   * the child model or record.
   * 
   * <p/>
   * This method will throw a {@link NotAssociatedException} in case a model that has no relationship is passed.
   * 
  ...
  public void remove(Model child) {
    ...
    if (child.frozen() || child.getId() == null)
      throw new IllegalArgumentException("Cannot remove a child that does " +
  ...
  /**
   * This method will not exit silently like {@link #save()}, it instead will throw
   * {@link org.javalite.activejdbc.validation.ValidationException} if validations did not pass.
   * 
   ...
   * 
   * @param query
   *          query to select records to count.
   * @param params
   *          parameters (if any) for the query.
  ...
  public static <T extends Model> Long count(Class<T> cls, String query, Object... params) {
  ...
  static <T extends Model> T instance(Map m, MetaModel metaModel) {
    try {
      ...
      throw new InitException("Failed to create a new instance of: " + metaModel.getModelClass()
          + ", are you sure this class has a default constructor?");
    } catch (DBException e) {
      throw e;
  ...
  public static <T extends Model> String getTableName(Class<T> claz) {
    return Registry.instance().getTableName(claz);
  }
  ...
  public static <T extends Model> void purgeCache(Class<T> cls) {
    MetaModel mm = getMetaModel(cls);
    if (mm.cached()) {
      QueryCache.instance().purgeTableCache(mm.getTableName());
    }
  ...
  private static <T extends Model> void purgeEdges(Class<T> cls) {
    ModelDelegate.purgeEdges(getMetaModel(cls));
  }
  ...
  public static <T extends Model> ModelContext<T> with(Class<T> cls) {
    return new ModelContext<T>(cls);
  }
