...
import org.javalite.activejdbc.cache.*;
...
public enum Registry {
   ...
   * 
   * @param table
   *          name of table represented by this MetaModel.
  ...
  private void processOverrides(List<Class<? extends Model>> models) {
    ...
    for (Class<? extends Model> modelClass : models) {
      ...
      if (many2manyAnnotation != null) {
        ...
        String thisPk;
        try {
          Method m = modelClass.getMethod("getMetaModel", Class.class);
          MetaModel mm = (MetaModel) m.invoke(modelClass, modelClass);
          thisPk = mm.getIdName();
          m = otherClass.getMethod("getMetaModel", Class.class);
          mm = (MetaModel) m.invoke(otherClass, otherClass);
  ...
  private Map<String, ColumnMetadata> getColumns(ResultSet rs, String dbProduct) throws SQLException {
    ...
    while (rs.next()) {
      ...
      ColumnMetadata cm = new ColumnMetadata(rs.getString("COLUMN_NAME").toLowerCase(), rs.getString("TYPE_NAME"),
   ...
   * 
   * @param join
   *          - potential name of a join table.
   * @param source
   ...
   * 
   * @param source
   *          name of table for which associations are searched.
   */
  ...
  public static String findTableName(Class<? extends Model> modelClass) {
